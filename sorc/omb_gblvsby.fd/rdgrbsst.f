...
! Arguments:
      INTEGER lupb, im, jm
      INTEGER iyrstmp, imstmp, idstmp, iret
      REAL anal(im,jm)
...
!Local:
      ...
      logical*1 lb(im*jm)
      real f(im*jm)
      INTEGER i, j, jf, kf, k, ni, nj
      INTEGER lat1, lon1, lati, loni
      INTEGER intrval, ioffset, mscan
      INTEGER kb1, kb2, kb3
      INTEGER iincdir, jincdir
c                                                                       
...
C       anal(i,j)=f(k)-273.15
        anal(i,j)=f(k)
      enddo
      PRINT *,'RG: max min retrieved sst ',MAXVAL(anal), MINVAL(anal)
