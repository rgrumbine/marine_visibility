C
      SUBROUTINE GRBFOG(IFT,F,IDX,IM,JM,MXBIT,ILPDS,IYR,IMO,IDA,IHR,
     1                  IYC,IMJM,MGRIB,GRIB,LGRIB,IGRIB,IERR)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C
C SUBPROGRAM:    GRBFOG      CREATE GRIB MESSAGE
...
C
C USAGE:    CALL GRBFOG(IFT,F,IDX,IM,JM,MXBIT,ILPDS,IYR,IMO,IDA,IHR,
C    &                  IYC,IMJM,MGRIB,GRIB,LGRIB,IERR)
C
...
C
C   OUTPUT FILES:
C     fort.98  - FILE FOR PRINTOUT
C     fort.68  - GRIBBED NH 1X1 DEG VISUAL RANGE DATA-ALL HOURS
C
...
!Arguments
      INTEGER IFT, IDX, IM, JM, MXBIT, ILPDS, IYR, IMO, IDA, IHR
      INTEGER IYC, IMJM, MGRIB, LGRIB, IGRIB, IERR
      CHARACTER*4 GRIB(MGRIB)
      REAL*4 F(IMJM)
   
      INTEGER igds, ids, ibds, itp, ipf, icp, ibf, in
...
C
      INTEGER IBMS
      DATA IBMS/0/,JPDS/03,06,09,12,15,18,21,24,27,30,33,36,39,42,45,
     ...
     4                168/
C
      INTEGER i, iscale, igrid, igf, nbm, nbit, nfo, ia
      REAL fmin, fmax
C
...
CXXXXNB----------------------------------------------------------------E
     ...
     1            IBM,IMJM,KBDS,NFO,GRIB,LGRIB,IERR)
      IF(IFT.EQ.68)THEN
         CALL BAWRITE(68,IGRIB,LGRIB,IA,GRIB)
         IGRIB=IGRIB+IA
         PRINT *,'IERR2ND',IERR,LGRIB,IA,IGRIB
      ENDIF
C
      RETURN
      END
